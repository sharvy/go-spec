...
function readCurrentTask(specDir) {
  ...
  try {
    ...
    debugLog(`readCurrentTask: ${err.message}`);
    return null;
  }
}
...
function readActiveTodo(specDir) {
  ...
  try {
    ...
    debugLog(`readActiveTodo: ${err.message}`);
    return null;
  }
}
...
function buildStatusline(input) {
  ...
  // Workspace root determines bridge file path (workspace isolation)
  const projectRoot = specDir ? path.dirname(specDir) : cwd;
  const bridgeFile = path.join(os.tmpdir(), `go-spec-context-${workspaceHash(projectRoot)}.json`);
  ...
  if (specDir) {
    ...
    // Project path
    parts.push(shortenPath(projectRoot));
  } else {
  ...
  if (remaining !== null) {
    ...
    // Write bridge file for context-monitor (workspace-isolated)
    try {
      fs.writeFileSync(bridgeFile, JSON.stringify({ usedPct: used, ts: Date.now() }));
    } catch (err) {
      debugLog(`writeBridgeFile: ${err.message}`);
    }
  }
