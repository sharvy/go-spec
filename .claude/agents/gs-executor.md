...
2. Read `.spec/PROJECT.md` for project-level constraints.
3. Read `.spec/STATE.md` for established patterns (naming, structure, conventions).
4. Read the pre-conditions — verify they're satisfied. If not, stop immediately and report.
5. Explore referenced files to understand what you're building on.
</step>
...
4. **Commit** atomically:
   ...
   git commit -m "plan [NN-PP] task [N]: [task name]"
   ```
...
- Discovered a conflict with another plan's recently committed code
...
After completing all tasks, write `.spec/phases/NN-[phase]/NN-PP-SUMMARY.md`.
</step>
</execution_flow>

<summary_format>
```markdown
# Summary: Plan [NN-PP] — [Plan Name]
...
## Commits
- `[sha]` — plan 01-01 task 1: [name]
- `[sha]` — plan 01-01 task 2: [name]
...
## Integration Points
[Key exports, interfaces, or contracts other plans/phases must know about]
